...
class TpcSoftwareGraduationReportMetric < ApplicationRecord
  ...
  def self.get_compliance_dco(project_url,oh_commit_sha)
    indexer, repo_urls =
      select_idx_repos_by_label_and_level(project_url, "repo", GiteeGitEnrich, GithubGitEnrich, GitcodeGitEnrich)
  ...
  def self.get_ecology_issue_management(project_url)
    indexer, repo_urls =
      select_idx_repos_by_label_and_level(project_url, "repo", GiteeIssueEnrich, GithubIssueEnrich, GitcodeIssueEnrich)
  ...
  def self.get_ecology_code_review(project_url)
    indexer, repo_urls =
      select_idx_repos_by_label_and_level(project_url, "repo", GiteePullEnrich, GithubPullEnrich, GitcodePullEnrich)
    base = indexer.must(terms: { tag: repo_urls})
